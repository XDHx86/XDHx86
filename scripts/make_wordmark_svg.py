"""
Render "XDHx86" as an EXTRUDED 3D wordmark rasterized to ASCII, and emit it as an
...
FONT_PATH = os.environ.get("WORDMARK_FONT", r"C:\Windows\Fonts\GOTHIC.ttf")
...
TEXT = os.environ.get("WORDMARK_TEXT", "XDHx86")

MASK_H = 300           # glyph raster height in mask px (drives voxel density)
TRACKING = 0.06        # extra letter-spacing, in em. counter gaps must survive the
...
DEPTH_FRAC = 0.20      # extrusion depth as a fraction of glyph height
TILT_DEG = float(os.environ.get("WORDMARK_TILT", 1.5))
...
def emit(frames, mode, out, dur, reveal):
    ...
    p.append(f'<text x="{canvas_w/2:.0f}" y="{TITLEBAR_H/2 + 4:.0f}" fill="{TITLE_TEXT}" '
             f'font-size="11.5" text-anchor="middle">XDHx86@github: ~$ ./wordmark.sh --3d</text>')
